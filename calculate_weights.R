...
qaly <- data %>% dplyr::select(pat_id,redcap_event_name,vst_dt,visit_date,visit_month,visit_year,mobility=spe_pp_mobility,selfcare=spe_pp_selfcare,activity=spe_pp_activities,pain=spe_pp_pain,anxiety=spe_pp_anxiety,healthtoday=spe_pp_health,outc_tx_decision,outc_eligible,model,cured) %>%
  mutate(
...
qalyinitial2 <- qalyinitial_dist %>% select(pat_id,thai_utility,indo_utility,stage,dem_age,dem_sex,treated,started_treatment,cured=cured.x,model=model.x) %>% mutate(point="initial")
...
qalytreatment <- qalyweighted %>% filter(language=="Khmer" & outc_tx_decision %in% c("C","W","M","TF") & !is.na(indo_utility)) %>% group_by(pat_id) %>% arrange(pat_id,visit_date) %>% mutate(date = visit_date-first(visit_date),last_date=as.numeric(last(visit_date)-first(visit_date))) %>% summarise(thai_utility=ifelse(n()>1,trapz(date,thai_utility)/last_date,thai_utility),indo_utility=ifelse(n()>1,trapz(date,indo_utility)/last_date,indo_utility),n=n(),point="treatment") %>% left_join(fstage,by="pat_id") %>% filter(!is.na(stage))
...
qalySVR_dist <- qalyweighted %>% filter(language=="Khmer" & outc_eligible=="FUP" & !is.na(indo_utility))  %>% group_by(pat_id) %>% arrange(pat_id,visit_date) %>% mutate(date = visit_date-first(visit_date),last_date=as.numeric(last(visit_date)-first(visit_date))) %>% summarise(thai_utility=ifelse(n()>1,trapz(date,thai_utility)/last_date,thai_utility),indo_utility=ifelse(n()>1,trapz(date,indo_utility)/last_date,indo_utility),n=n(),point="SVR") %>% left_join(fstage,by="pat_id") %>% filter(!is.na(stage))
...
qalyall <- bind_rows(qalyinitial2,qalytreatment,qalySVR_dist) 
# save(qalyall,file="~/dat/Cambodia_data/qalyall.RData")
